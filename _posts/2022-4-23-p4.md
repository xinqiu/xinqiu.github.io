...
```bash
docker run --entrypoint /bin/sh -p 30303:30303 -p 30303:30303/udp -p 8545:8545 -it -v $(pwd):/root --privileged=true --name eth-main ethereum/client-go
```
...
```
docker run --entrypoint /bin/sh -it -v $(pwd):/root --privileged=true --name eth-slave ethereum/client-go
```
...
刚才虽然启动了节点，但是由于开启了`--nodiscover`，节点无法直接通过发现的方式自动互联，因此需要手动进行添加节点。通过查询node的信息，然后使用`admin.addPeer()`添加节点。

![img](/assets/pic/geth-2.png)

实际上，即使`admin.addPeer()`返回`True`也不代表真的就添加节点成功，需要通过`admin.peers`进行检查。
